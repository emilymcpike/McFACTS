...
def circular_binaries_encounters_ecc_prograde_old(
        smbh_mass,
        disk_bh_pro_orbs_a,
        disk_bh_pro_masses,
        disk_bh_pro_orbs_ecc,
...
def circular_binaries_encounters_ecc_prograde_obj(
    ...
    bindex = blackholes_binary.num + 1

    disk_bins_bhbh = circular_binaries_encounters_ecc_prograde_old(smbh_mass, disk_bh_pro_orbs_a, disk_bh_pro_masses,
...
def circular_binaries_encounters_circ_prograde_old(
        smbh_mass,
        disk_bh_pro_orbs_a,
        disk_bh_pro_masses,
        disk_bh_pro_orbs_ecc,
...
def circular_binaries_encounters_circ_prograde_obj(
    ...
    disk_bins_bhbh = circular_binaries_encounters_circ_prograde_old(smbh_mass, disk_bh_pro_orbs_a, disk_bh_pro_masses,
                                                                disk_bh_pro_orbs_ecc, timestep_duration_yr,
                                                                disk_bh_pro_orb_ecc_crit, delta_energy_strong,
                                                                disk_bins_bhbh, bindex)
...
def bin_spheroid_encounter_old(
        smbh_mass,
        timestep_duration_yr,
        disk_bins_bhbh,
        time_passed,
    ...
    rng_enc_arr = np.array([])
    rng_rad_arr = np.array([])
    rng_mss_arr = np.array([])
    rng_ii3_arr = np.array([])
    ...
    for i in range(0,number_of_binaries-1):
        ...
        random_uniform_number = rng.uniform(size=1)
        rng_enc_arr = np.concatenate((rng_enc_arr, random_uniform_number))
        if random_uniform_number < enc_rate:
            ...
            rng_rad_arr = np.concatenate((rng_rad_arr, random_uniform_number2))
            ...
            #mass_3 = (rng.pareto(nsc_bh_imf_powerlaw_index, size=1) + 1) * mode_star
            random_uniform_number3 = rng.pareto(nsc_bh_imf_powerlaw_index, size=1)
            rng_mss_arr = np.concatenate((rng_mss_arr, random_uniform_number3))
            mass_3 = (random_uniform_number3 + 1) * mode_star
            # K.E_3 in Joules
            # Keplerian velocity of ecc prograde orbiter around SMBH (=c/sqrt(a/r_g))
            ...
            if time_passed < crit_time:
                if radius_3 < crit_radius:
                    ...
                    rng_ii3_arr = np.concatenate((rng_ii3_arr, np.array([i3])))
    ...
    #return disk_bins_bhbh
    return (disk_bins_bhbh, rng_enc_arr, rng_rad_arr, rng_mss_arr, rng_ii3_arr)
...
def bin_spheroid_encounter_obj(
    ...
    disk_bins_bhbh, rng_enc_arr, rng_rad_arr, rng_mss_arr, rng_ii3_arr = bin_spheroid_encounter_old(smbh_mass, timestep_duration_yr, disk_bins_bhbh, time_passed, bindex,
    ...
    return (blackholes_binary, rng_enc_arr, rng_rad_arr, rng_mss_arr, rng_ii3_arr)
